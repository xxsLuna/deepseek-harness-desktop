...
- **Desktop integration** — tray icon, notifications for approvals and finished turns (subagent turns kept separate, and off), a rebindable global shortcut, window snapping to screen edges, deep links, auto-update (Windows/Linux).
...
- **Desktop preferences** live with the launcher, not the harness: what closing the window does, which events raise an OS notification, whether the title bar is merged, whether the app updates itself, whether the window snaps to screen edges, and which chord shows or hides it. `@dsh-desktop/settings` renders them as Settings sections and reads and writes them over the same `/__desktop-host/` route the band's controls use. All of them apply immediately except the title bar, which is a window-construction option, so changing it says so and applies on restart.

- **Usage** is counted by the launcher, from the event stream it already reads for notifications, and stored in `usage.json` beside the preferences. Nothing upstream keeps a time series — the harness's own `sessionStats` is per-session totals with no timestamps — so the record begins the day you install this and there is no history to fill in from before that; the page says as much rather than showing an empty year. Both graphs read one table of local day-and-hour buckets, so they cannot disagree, and **Reset** empties it.
